...
# News 
* 2022.3.21: Released the pretrining code and datasets of OFA.
...
1. **Prepare the Dataset & Checkpoints**: To pretrain OFA, you should first download the dataset we have provided (pretrain_data_examples.zip, a small subset of the original pretraining data). pretrain_data_examples.zip contains 4 tsv files vision_language_examples.tsv, text_examples.tsv, image_examples.tsv and detection_examples.tsv. Details of these files are as follows:

vision_language_examples.tsv:
uniq_id, image(base64 string), caption, question, answer, gt_objects(objects that appearing in the caption or question)，dataset_name(source of the data)，type(caption, qa or visual gronunding)

text_examples.tsv:
uniq_id, text

image_examples.tsv:
uniq_id, image(base64 string), image_code(generated by VQ-GAN)

detection_examples.tsv:
uniq_id, image(base64 string), label

In addition, the folder negative_sample in pretrain_data_examples.zip contains three files all_captions.txt, object.txt and type2ans.json. The data in these files are used as negative samples for the ITM task.

2. **Pretraining**: We recommend to load ofa_large or ofa_base for continue pretraining to achieve better results.
``
