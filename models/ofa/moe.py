...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, gates, device):
        ...
        # drop indices
        print('debug -> gates:{}'.format(gates))
        print('debug -> sorted_experts:{}'.format(sorted_experts.shape))
...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, output_size, num_experts, hidden_size, ffn_layernorm, dropout_module, activation_dropout_module, noisy_gating, k, device):
        ...
        self.k = k
        # instantiate experts
        self.experts = nn.ModuleList([MLP(self.input_size, self.output_size, self.hidden_size, ffn_layernorm, dropout_module, activation_dropout_module) for i in range(self.num_experts)])
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        """
        self.w_gate = self.w_gate.half()
        self.w_noise = self.w_noise.half()
