...
class SGRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        self.result_dict[mode + '_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
        self.result_dict[mode + '_recall_collect'] = {20: [], 50: [], 100: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_recall'].items():
            result_str += '    R @ %d: %.4f; ' % (k, v)
        result_str += ' for mode=%s, type=Recall(Main).' % mode
        result_str += '\n'
        return result_str
    
    def calculate_recall(self, mode):
        for k, v in self.result_dict[mode + '_recall_collect'].items():
            self.result_dict[mode + '_recall'][k] = np.mean(v)

    def collect_recall(self, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + '_recall']:
            ...
            self.result_dict[mode + '_recall_collect'][k].append(rec_i)
